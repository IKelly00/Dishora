...
@section('content')
    ...
    <div class="container py-4">
        <div class="main-content-area">
            ...
            <form action="{{ route('vendor.menu.update', $product->product_id) }}" id="editItemForm" method="POST"
                    ...
                    <div class="col-lg-5">
                        <div class="border rounded p-3 w-100">
                            ...
                            <div class="upload-box mb-3" id="uploadBox">
                                ...
                                @php $hasImage = !empty($product->image_url); @endphp
                                ...
                                <div id="item_image_preview"
                                    class="{{ $hasImage ? '' : 'd-none' }} position-absolute w-100 h-100 top-0 start-0">
                                    @if ($hasImage)
                                        <img src="{{ $product->image_url }}" alt="Preview" class="w-100 h-100"
                            ...
                            <div class="input-group input-group-sm mb-3">
                                ...
                                <input type="text" id="item_file_name" class="form-control"
                                    value="{{ $hasImage ? basename($product->image_url) : 'None' }}" readonly
                                    data-original="{{ $hasImage ? basename($product->image_url) : 'None' }}">
                            </div>
                    ...
                    <div class="col-lg-7">
                        ...
                        <div class="border-bottom mb-3 pb-2">
                                ...
                                <select name="category" id="itemCategory" class="form-select" required>
                                    ...
                                    @foreach ($categories as $cat)
                                        <option value="{{ $cat->product_category_id }}"
                                            {{ old('category', $product->product_category_id) == $cat->product_category_id ? 'selected' : '' }}>
                                            {{ $cat->category_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="price" id="itemPrice" class="form-control" step="0.01"
                                    min="1" value="{{ old('price', $product->price) }}" required>
                            </div>
                        </div>
                        ...
                        <div class="border-bottom mb-3 pb-2">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="availability" id="itemAvailability" class="form-select" required>
                                    <option value="Available" {{ $product->is_available ? 'selected' : '' }}>Available
                                    </option>
                                    <option value="Not Available" {{ !$product->is_available ? 'selected' : '' }}>Not
                                        Available</option>
                                </select>
                            </div>
                                ...
                                <select id="dietSelect" name="diet[]" multiple>
                                    @foreach ($dietarySpecs as $spec)
                                        <option value="{{ $spec->dietary_specification_id }}"
                                            {{ in_array($spec->dietary_specification_id, old('diet', $selectedSpecs ?? [])) ? 'selected' : '' }}>
                        ...
                        <div class="border-bottom mb-3 pb-2">
                            ...
                            <div id="cutoffWrapper" class="mb-3">
                                ...
                                <small class="text-muted">Set time buffer before closing (15-min increments).</small>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="itemDescription" rows="3" class="form-control" required>{{ old('description', $product->description) }}</textarea>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            cancelBtn.addEventListener('click', () => {
                ...
                fileNameDisplay.value = fileNameDisplay.dataset.original;
            });
            ...
            const cutoffHours = document.getElementById('cutoffHoursInput');
            ...
            function toggleCutoff() {
                const isPreorder = preorderYes.checked;
                cutoffHours.disabled = isPreorder;
                cutoffMinutes.disabled = isPreorder;
                if (isPreorder) {
                ...
                advance.disabled = !isPreorder;
                advanceWrapper.style.opacity = isPreorder ? '1' : '0.6';
                advanceWrapper.style.pointerEvents = isPreorder ? 'auto' : 'none';
                if (!isPreorder) advance.value = (0).toFixed(2);
            }
            preorderYes.addEventListener('change', toggleCutoff);
            preorderNo.addEventListener('change', toggleCutoff);
            toggleCutoff();
        });
    </script>
@endsection
